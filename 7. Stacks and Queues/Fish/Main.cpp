...
Arrays A and B represent N voracious fish in a river, ordered downstream along the flow 
of the river.
...
Fish number P is represented by A[P] and B[P]. Array A contains the sizes of the fish. All 
...
eats the smaller one. More precisely, we say that two fish P and Q meet each other 
...
Initially all the fish are alive and all except fish number 1 are moving upstream. Fish 
number 1 meets fish number 2 and eats it, then it meets fish number 3 and eats it too. 
...
that, given two non-empty zero-indexed arrays A and B consisting of N integers, returns 
...
Results (100%): 
https://app.codility.com/demo/results/training3QPM9G-XAN/
...
{
	std::stack< size_t > alive;
	for ( size_t fish = 0; fish < A.size(); fish++ ) {
		if ( B[ fish ] == 1 ) {
			// downstream
			alive.push( fish );
		}
		else {
			while ( !alive.empty() && B[ alive.top() ] == 1 && A[ alive.top() ] < A[ fish ] ) {
				alive.pop();
			}

			if ( alive.empty() || B[ alive.top() ] == 0 ) {
				alive.push( fish );
			}
		}
	}
...
void test( std::vector< int > A, std::vector< int > B, int expected )
{
	static size_t testCount = 0;

	std::cout << "Test " << testCount << ": ";
	auto got = solution( A, B );
	if ( got != expected ) {
