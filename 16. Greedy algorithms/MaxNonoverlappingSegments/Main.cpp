...
zero-indexed arrays A and B. For each I (0 ≤ I < N) the position of segment I is from A[I] 
to B[I] (inclusive). The segments are sorted by their ends, which means that B[K] ≤ B[K + 1] 
...
that, given two zero-indexed arrays A and B consisting of N integers, returns the size 
of a non-overlapping set containing a maximal number of segments.
...
Results (100%):
https://app.codility.com/demo/results/trainingTFH87T-2TR/
...
{
	codility::compareResults( solution( A, B ), expected );
}

int main( int argc, char **argv )
{
	test( { 1, 3, 7, 9, 9 }, { 5, 6, 8, 9, 10 }, 3 );
	test( { 2, 1, 3, 5 }, { 4, 5, 6, 8 }, 2 );
